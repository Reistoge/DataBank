...
export class MerchantRepository {
  ...
  async getMerchantbyName(merchantName: string): Promise<MerchantDocument> {
    if (!merchantName || typeof merchantName !== 'string') {
      throw new BadRequestException(
        'Merchant name is required and must be a string',
      );
    }

    try {
      const merchant = await this.merchantModel
        .findOne({ name: merchantName })
        .lean<Merchant>()
        .exec();

      if (!merchant) {
        throw new NotFoundException(
          `Merchant '${merchantName}' does not exist`,
        );
      }
      ...
      this.logger.error(
        `Error getting merchant by name '${merchantName}': ${error?.message}`,
      );
  ...
  async create(dto: CreateMerchantDto): Promise<MerchantResponseDto> {
    if (!dto?.name || !dto?.accountNumber || !dto?.email) {
      throw new BadRequestException(
        'Name, account number, and email are required',
      );
    }
    ...
    try {
      ...
      if (existingMerchant) {
        throw new ConflictException(
          `Merchant with name '${dto.name}' already exists`,
        );
      }
  ...
  async getMerchant(
    ...
    if (!name && !accountNumber) {
      throw new BadRequestException(
        'Either name or accountId must be provided',
      );
    }
  ...
  async findMerchantProductDocument(
    accountNumber: string,
    productsProps?: Partial<Product>,
  ): Promise<Product[]> {
    if (!accountNumber) {
    ...
    try {
      ...
      const filter = {
        ...Object.fromEntries(
          Object.entries(productsProps || {}).filter(
            ([, v]) => v !== undefined,
          ),
        ),
  ...
  async findMerchantDocument(
    ...
    try {
      const filter = {
        ...Object.fromEntries(
          Object.entries(merchantProps || {}).filter(
            ([, v]) => v !== undefined,
          ),
        ),
      };
