...
import {
  ...
  NominatimLocationResponseDto,
} from './dto/nominatim.dto';
...
export class GeolocationService {
  ...
  public async getLocationData(
    dto: NominatimLocationRequestDto,
  ): Promise<NominatimLocationResponseDto> {
    try {
      ...
      const response: AxiosResponse<NominatimLocationResponseDto[]> =
        await this.httpService.axiosRef.get<NominatimLocationResponseDto[]>(
          url,
      ...
      if (!Array.isArray(data) || data.length === 0) {
        this.logger.warn(
          `Nominatim returned no results for: ${params.toString()}`,
        );
        throw new NotFoundException('No geolocation result');
      }

      return data[0]; // return the first (and only) result
    } catch (error) {
      this.logger.error(
        `Geolocation lookup failed: ${error?.message || error}`,
      );
      throw error;
    }
  }
  ...
  async calculateDistanceBetween(
    dtoA: NominatimLocationRequestDto,
    dtoB: NominatimLocationRequestDto,
  ) {
