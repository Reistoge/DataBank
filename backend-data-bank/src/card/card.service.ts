...
export class CardService {
  async getCardByCardNumber(cardNumber: string): Promise<CardDocument> {
    try {
      ...
      this.logger.log(
        `card document found for ${cardNumber} returning document`,
      );
      return doc;
    } catch (err) {
      ...
      throw err instanceof Error
        ? err
        : new Error(`Error while getting card by his number property`);
    }
  }
  async validateCard(
    cardValidationCredential: CardValidationCredential,
  ): Promise<{
    ...
    try {
      const accountDoc: AccountDocument =
        await this.accountService.getAccountDocumentFromCardNumber(
          cardValidationCredential.cardNumber,
        );
      ...
      if (userDoc.userNumber !== cardValidationCredential.user.userNumber) {
        throw new UnauthorizedException(
          `card number ${cardValidationCredential.cardNumber} corresponds to another user to user ${cardValidationCredential.user}`,
        );
      ...
      throw err instanceof Error
        ? err
        : new Error(
            `Error in validation card process for card number: ${cardValidationCredential.cardNumber} and user ${cardValidationCredential.user} `,
          );
    }
  }
  ...
  async create(createCardDto: CreateCardDto): Promise<CardResponse> {
    ...
    try {
      ...
      const q: CypherQuery<CardDocument> = new CreateCardNode(
        this.neo4jService,
        createdCard,
      );
      q.execute();
  ...
  async findCardsByAccountId(accountId: string): Promise<CardResponse[]> {
    const cardDocs = await this.cardModel
      .find({ accountId, state: CardState.DEFAULT })
      .exec();
  ...
  async findCardsDocumentByAccountId(
    accountId: string,
  ...
  async updateCard(
    ...
    updateCardDto: UserUpdateCardReqDto,
  ): Promise<void> {
  ...
  async deleteCard(cardId: string, password: string): Promise<void> {
    ...
    await this.cardModel
      .updateOne({ _id: cardId }, { state: CardState.DELETED })
      .exec();
  ...
  async deleteCardsForAccount(accountId: string): Promise<void> {
    ...
    await this.cardModel
      .updateMany({ accountId }, { state: CardState.DELETED })
      .exec();
  }
