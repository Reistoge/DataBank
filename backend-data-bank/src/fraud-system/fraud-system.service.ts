...
import {
  CreateInvalidTransactionNode,
  CreateTransactionNode,
} from './queries/cypher-query';
...
export class FraudSystemService {
  ...
  async validate(txDocument: TransactionDocument): Promise<FraudResult> {
    try {
      ...
      const behaviours: SuspiciousBehaviour[] =
        await this.validator.runAll(txDocument);

      this.logger.log(
        `validations finished, tx behaviours: ${JSON.stringify(behaviours)}`,
      );
      const riskScore =
        SuspiciousBehaviourAnalyzer.calculateRiskScore(behaviours);
      const reccomendation =
        SuspiciousBehaviourAnalyzer.getRecommendation(riskScore);
  ...
  async createInvalidTransactionNode(txDocument: TransactionDocument) {
    try {
      const query = new CreateInvalidTransactionNode(
        this.neo4jService,
        txDocument,
      );
      const result = await query.execute();
