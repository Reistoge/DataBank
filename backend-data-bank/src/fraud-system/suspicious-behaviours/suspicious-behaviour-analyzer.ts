...
export class SuspiciousBehaviourAnalyzer {
    static calculateRiskScore(behaviors: SuspiciousBehaviour[]): number {
        ...
        if (hasCritical) return Math.min(1.0, totalWeight * diversityMultiplier * 1.5);
    ...
    static getHighestSeverity(behaviors: SuspiciousBehaviour[]): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' | number {
        let highest = 0;
