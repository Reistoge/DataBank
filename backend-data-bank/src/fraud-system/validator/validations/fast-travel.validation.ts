...
export class FastTravelValidation extends TransactionValidation {
  ...
  async validate(tx: TransactionDocument): Promise<SuspiciousBehaviour[]> {
    ...
    try {
      ...
      if (!location) {
        this.logger.log('FastTravelValidation: no location data in transaction');
        return behaviours;
      }
      ...
      if (!currentLocation) {
        this.logger.warn(`FastTravelValidation: could not parse location: ${location}`);
        return behaviours;
      }
      ...
      if (recentTxs.length === 0) {
        ...
        const referenceLocation = await this.getAccountReferenceLocation(senderAccountNumber);
      ...
      this.logger.error(`FastTravelValidation error: ${error?.message || error}`);
      return [];
    }
  }
  ...
  private async getRecentTransactions(senderAccountNumber: string): Promise<Array<{
    ...
    try {
      ...
      const locationTxs: Array<{ location: LocationPoint; time: Date; transactionId: string }> = [];
      ...
      for (const histTx of recentTxs) {
        try {
          ...
          this.logger.warn(`Failed to parse historical transaction location: ${error?.message}`);
        }
      }
  ...
  private async getAccountReferenceLocation(senderAccountNumber: string): Promise<LocationPoint | null> {
    try {
      // Get user data for fallback to user's registered location
      const user = await this.accountService.getUserDocumentByAccountNumber(senderAccountNumber);
      ...
      const account = await this.accountService.getAccountDocumentByAccountNumber(senderAccountNumber);
      ...
      this.logger.error(`Failed to get account reference location: ${error?.message}`);
      return null;
    }
  }
  ...
  private async checkTravelFromReference(
    ...
    try {
      const distance = await this.locationService.calculateDistanceBetweenPoints(
        referenceLocation,
        currentLocation
      ) as number;
      ...
      if (requiredSpeed > this.MAX_REASONABLE_SPEED_KMH) {
        ...
        this.logger.log(`FastTravel detected from reference: ${distance.toFixed(0)}km in ${assumedTimeGap}h = ${requiredSpeed.toFixed(0)}km/h`);
      }
    } catch (error) {
      this.logger.error(`Error checking travel from reference: ${error?.message}`);
    }
  }
  ...
  private async checkFastTravelFromHistory(
    currentLocation: LocationPoint,
    currentTime: Date,
    recentTxs: Array<{ location: LocationPoint; time: Date; transactionId: string }>,
    behaviours: SuspiciousBehaviour[]
  ): Promise<void> {
    try {
      ...
      if (requiredSpeed > this.MAX_REASONABLE_SPEED_KMH) {
        ...
        behaviours.push(behaviour);
        this.logger.log(`FastTravel detected: ${distance.toFixed(0)}km in ${timeGapHours.toFixed(1)}h = ${requiredSpeed.toFixed(0)}km/h`);
      }
      ...
      for (let i = 1; i < Math.min(recentTxs.length, 3); i++) {
        ...
        const olderDistance = await this.locationService.calculateDistanceBetweenPoints(
          olderTx.location,
          currentLocation
        ) as number;
        ...
        if (olderTimeGapHours >= this.MIN_TIME_GAP_HOURS && olderDistance >= this.MIN_DISTANCE_FOR_CHECK) {
          ...
          if (olderRequiredSpeed > this.MAX_HUMAN_SPEED_KMH) {
            ...
            behaviours.push(behaviour);
            this.logger.log(`FastTravel pattern detected: ${olderDistance.toFixed(0)}km in ${olderTimeGapHours.toFixed(1)}h = ${olderRequiredSpeed.toFixed(0)}km/h`);
          }
        }
      }

    } catch (error) {
      this.logger.error(`Error checking fast travel from history: ${error?.message}`);
    }
  }
}