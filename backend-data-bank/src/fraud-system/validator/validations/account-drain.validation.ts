...
export class AccountDrainValidation extends TransactionValidation {
  ...
  async validate(tx: TransactionDocument): Promise<SuspiciousBehaviour[]> {
    ...
    try {
      ...
      if (!snapshot?.request) {
        this.logger.warn('AccountDrainValidation: missing transaction snapshot');
        return [];
      }
      ...
      if (typeof amount !== 'number' || amount <= 0) {
        this.logger.warn('AccountDrainValidation: invalid amount in transaction');
        return [];
      }

      if (!senderAccountNumber) {
        this.logger.warn('AccountDrainValidation: missing sender account number');
        return [];
      }

      // Get current account balance
      const currentBalance = await this.getCurrentAccountBalance(senderAccountNumber, snapshot);
      
      if (currentBalance === null) {
        this.logger.warn(`AccountDrainValidation: could not get balance for account ${senderAccountNumber}`);
        return [];
      }

      // Skip validation for very small balances to avoid false positives
      if (currentBalance < this.MINIMUM_BALANCE_CHECK) {
        this.logger.log(`AccountDrainValidation: balance too small (${currentBalance}) for meaningful drain analysis`);
        return [];
      }
      ...
      this.logger.log(`AccountDrainValidation: ${JSON.stringify(drainAnalysis)}`);
      ...
      if (drainAnalysis.drainPercentage >= this.MODERATE_DRAIN_THRESHOLD) {
        ...
        this.logger.log(`AccountDrain detected: ${drainAnalysis.drainLevel} - ${(drainAnalysis.drainPercentage * 100).toFixed(1)}%`);
        return [behaviour];
      }
      ...
      this.logger.error(`AccountDrainValidation error: ${error?.message || error}`);
      return [];
    }
  }

  private async getCurrentAccountBalance(senderAccountNumber: string, snapshot: any): Promise<number | null> {
    try {
      // Try to get balance from snapshot first (more accurate as it's the balance at transaction time)
      if (snapshot.senderAccount?.balance !== undefined && snapshot.senderAccount.balance !== null) {
        return snapshot.senderAccount.balance;
      }

      // Fallback to current balance from account service
      return await this.accountService.getAccountBalanceByAccountNumber(senderAccountNumber);
    } catch (error) {
  ...
  private analyzeDrain(amount: number, balance: number): {
    amount: number;
    balance: number;
